...
- [ ] **Persist window size/position between sessions** — restore the last window geometry on launch so the user doesn't resize every time. **Constraint (from the WSLg debugging notes):** naively persisting geometry was previously a footgun — restoring an off-screen or zero-size rect made the app appear to "hang" on launch. Guard against it: clamp/validate the restored rect against the current monitor work-area (and fall back to a sane default if it's off-screen or degenerate) before applying. Geometry persists via the same `persisted` state path as `ui_scale`/theme. — _added 2026-07-09_
...
- [x] **Interval-create → Enter opens label edit (cursor in field), Enter accepts, Esc cancels** — currently Enter on a just-created interval throws an overlap warning instead of entering label editing — _added 2026-07-02_ · **shipped 2026-07-09** (see DEVLOG; `enter_commit` now selects the new annotation + clears the selection). The broader keybindings pass + "chain interval from previous endpoint" stay open above.
- [x] **Bug: signal-lane y-axis labels clipped at the top** — rotated labels like "f0" lose the top of the glyphs; add a bit more left/top padding — _added 2026-07-02_ · **shipped 2026-07-09** (see DEVLOG; resolved via the `signal_lane` unification + `SIGNAL_AXIS_LABEL_PAD` inset, applied uniformly to every lane).
- [x] **Bug: "Recording too short for spectrogram window" shows mid-recording** — message bar reports it before the recording is finished — _added 2026-07-02_ · **shipped 2026-07-09** (see DEVLOG; live dispatcher guards on the real `window_samples`).
- [x] **Bug: View ▸ UI scale slider moves as it scales** — the slider repositions under the cursor while dragging, breaking the selector — _added 2026-07-02_ · **shipped 2026-07-09** (see DEVLOG; applied zoom frozen while dragging via `ui_scale_dragging`).

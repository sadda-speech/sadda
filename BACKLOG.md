...
- [ ] **GUI surface for adaptive pitch-range methods** — the follow-up to the engine+Python adaptive-pitch work (two-pass + complete-pooling + empirical-Bayes speaker methods). Add a unified **range-mode picker** to the pitch editor: Manual / Adaptive (two-pass) / Speaker-adaptive (pooled | empirical-Bayes). Two-pass fits the per-recording f0 lane directly (dispatch `compute_measure_tracks:7645` to `two_pass_pitch`, show the derived range as the lane y-bounds). The **speaker methods need a speaker-level UI home first** (a speaker panel / "compute speaker pitch range" action — relates to the backlogged Bundle metadata panel) + corpus auto-grouping by `bundle.speaker_id`. Deliberately deferred (user call 2026-07-09) to design the range-mode UI once, coherently, after the pending lane-refactor PR merges — avoids racing it. — _added 2026-07-09_

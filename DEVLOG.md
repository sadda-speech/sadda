...
**Sourcing + the C-vs-published check (as asked).** Camacho's page 403'd, so I pulled Kyle Gorman's MIT C port and, separately, Camacho's **own dissertation-appendix MATLAB** (`swipe.m`, bundled in Gorman's repo). Line-by-line, the C faithfully implements the MATLAB (ERB scale, `4·K=8` window sizing, 50 % hop, prime-harmonic kernel + `√(1/f)` envelope + `‖k₊‖` norm, `mu` blend, parabolic refine) — the **one** difference is the Hann window: the C uses a periodic Hann (denominator `N`), Camacho's MATLAB uses `hanning` (denominator `N+1`). Per the maintainer's call we implement the **author's** definition (`N+1`) and note the C's variant in the doc comment (negligible for SWIPE's large windows). With the MATLAB confirmed canonical, the C was set aside.

**Implementation** (`engine::pitch::swipe`, f64): a faithful port of the MATLAB with **no algorithmic deviation**. Includes a from-scratch **not-a-knot cubic spline** (`spline_moments_uniform`/`spline_eval_uniform`) for the magnitude→ERB resampling (`interp1(…,'spline')`) — initially I shortcut to linear interpolation, but per the maintainer's push ("why not the author's spline?") implemented the real thing; the uniform-grid not-a-knot end conditions decouple `M₁`/`M_{n-2}`, leaving a `1,4,1` tridiagonal solve, validated against scipy to machine-ε.

**Validation = the author's own code, and it earned its keep.** Golden generated by running Camacho's verbatim `swipep` under **Octave** (`tests/dsp/swipe/run_swipe_octave.m`, self-contained: local `hanning`/`specgram` so no signal-package install). The cross-check **caught a real bug**: my first port (and a numpy port) agreed with each other (149.56 Hz) but disagreed with the author (150.10 Hz) on the 150 Hz fixture. Root cause: Camacho's loop index is **1-based** and `d` is calibrated to it, but my Rust loop was 0-based → `d − i` off by one → candidates assigned to the wrong window + skewed `mu`. After the fix, Rust matches the Octave author-golden to **< 0.05 Hz** (`tests/swipe_pitch.rs`). The numpy port (which shared the bug) was dropped; `make_swipe_fixtures.py` now only emits the deterministic input tones, Octave owns the golden.

Also in-module: spline-vs-scipy unit test, harmonic-tone recovery, **cross-family agreement** (SWIPE'/YIN/Boersma within 5 Hz on a clean tone), and silence→unvoiced.

**Three surfaces:** engine + Python `sadda.dsp.voiced_pitch(method="swipe")` (+ test) + GUI unchanged (the existing pitch lane is method-agnostic, like YIN/pYIN/Boersma). Module doc promoted SWIPE' from "deferred alternates" to the implemented spectral family (six trackers now).

**Docs — external-tool validation (as asked).** New `crates/engine/tests/README.md` documents the validation philosophy and a table of which authoritative reference + tool produces each golden (Praat / librosa / OpenAI Whisper / Octave), making clear the suite runs offline against committed goldens and the tools are only for regeneration — modelling the rigor for contributors. README gains a short "Validation" subsection pointing there.

Gates green: fmt/clippy `--workspace --all-targets -D warnings`/`test --workspace` (incl. the Octave-golden + spline + cross-tracker tests), stubs regenerated (+`swipe` docstring bullet), 182 pytest + 27 `tools/docs`, source-link gate 271/0, `mkdocs build --strict` clean.

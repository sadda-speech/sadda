...
pub(crate) fn get(
    ...
    let pyproj = project.borrow();
    let r = pyproj.inner.recipe_by_name(name).map_err(engine_err_to_py)?;
    Ok(PyRecipe {
        id: r.id,
        name: r.name,
        sadda_version: r.sadda_version,
...
pub(crate) fn generate_script(
    ...
    fs::create_dir_all(&recipes_dir).map_err(|e| {
        sadda_engine::EngineError::Io(e)
    }).map_err(engine_err_to_py)?;
...
fn render_script(
    ...
    out.push_str(&format!("# Auto-generated by sadda.recipe.\n"));
    out.push_str(&format!("# Recipe name: {recipe_name}\n"));
    out.push_str(&format!("# Sadda version at record time: {sadda_version}\n"));
...
fn render_call(out: &mut String, run: &ProcessingRunRow) {
    ...
    match run.processor_id.as_str() {
        ...
        "sadda.live" => {
            out.push_str(
                "    # Live recordings cannot be replayed from a script (they\n",
            );
            out.push_str(
                "    # require physical audio input). The bundle exists; this is\n",
            );
...
/// Convenience for tests: returns the rendered script body without
/// touching the filesystem. Kept `pub(crate)` for cross-module unit
/// tests; not exposed to Python.
#[cfg(test)]
pub(crate) fn render_for_test(
    ...
    render_script(recipe_name, sadda_version, project_root, runs)
}

...
- [ ] **ASR + phone-level forced alignment (A1–A5)** — produces Words / Syllables / Phones tiers, "defaults that just work". **Designed 2026-07-05** (see DEVLOG): alignment-first · both engines (neural default + MFA passthrough) · IPA-multilingual via espeak-ng G2P. Model chosen: `facebook/wav2vec2-lv-60-espeak-cv-ft` (Apache-2.0, CTC, espeak-IPA) behind a `sadda[align]` extra. Architecture = ONNX acoustic posteriors (`sadda.ml`) + a constrained-Viterbi DP in the Rust `engine` (three surfaces). Slices: **A1** engine DP + espeak-ng G2P + ONNX phone model → Words+Phones + Python API (start by verifying espeak-ng↔model token match); **A2** MFA 3.0 passthrough (subprocess → TextGrid import); **A3** syllabification (sonority + maximal-onset); **A4** Whisper ASR (no-transcript path); **A5** GUI. QA via the S5 agreement engine (boundary Δ / κ). **A1 shipped 2026-07-06** (`sadda.align`: engine DP + espeak-ng G2P + `Wav2Vec2EspeakModel.from_pretrained("sadda-speech/wav2vec2-espeak-ctc")`, validated on real audio). A1 refinements: **silence detection prototyped 2026-07-06** (`detector="blank"` default / `"vad"`, empty-interval output; see DEVLOG + `feat/align-silence` draft PR) — remaining silence work: **truly modeled silence** (comes with A2/MFA, or a `sil`-aware neural model), and a **VAD∩blank hybrid** (VAD alone is coarse and can drop short boundary phones). **Input resampling shipped 2026-07-06** — `emissions()` resamples any-rate audio to 16 kHz via `Audio.resample` (engine FFT-domain resampler); mono input still expected. **A2 (MFA passthrough) shipped 2026-07-06** — `sadda.align.mfa_align` (single file via `mfa align_one`) + `mfa_align_corpus` (batch) → same `Alignment`; `import_alignment` writes any Alignment onto a bundle (backend-agnostic); modeled `sil`/`sp` kept **labelled** (vs A1's imputed empties); opt-in detect-or-error; exposed engine `parse_textgrid_intervals` + fixed a UTF-8/IPA TextGrid-tokenizer bug. **This delivers the "truly modeled silence" item** (remaining: `sil`-aware neural model, VAD∩blank hybrid). Remaining slices: **A3** syllabification · **A4** Whisper ASR (first-class — no-transcript conversational/naturalistic work, not "just convenience") · **A5** GUI. — _added 2026-07-02, groomed 2026-07-05_
